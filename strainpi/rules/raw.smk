rule raw_prepare_reads:
    input:
        lambda wildcards: strainpi.get_raw_input_list(wildcards, SAMPLES, DATA_TYPE)
    output:
    ...
    params:
        sample_id = "{sample}",
        input_files = lambda wildcards: strainpi.get_raw_input_dict(wildcards, SAMPLES, DATA_TYPE),
        headers = strainpi.HEADERS,
